...
height:this.$tn3.height()};this.$tn3.css("float","left");d.wDif=this.$tn3.outerWidth(true)-d.width;d.hDif=this.$tn3.outerHeight(true)-d.height;this.replaceMenu("","");var b=this;this.timer=new f.fn.tn3.Timer(this.$c,this.config.delay,this.config.timerSteps);this.$c.bind("timer_end",function(){b.show("next")});this.special={rv:[],rh:[],v:[],h:[],o:[]};this.parseLayout();this.center();f.each(this.items,function(g,i){switch(g){case "next":i.click(function(k){b.show("next");
k.stopPropagation()});i.attr("title",b.t("Следующие изображение"));break;case "prev":i.click(function(k){b.show("prev");k.stopPropagation()});i.attr("title",b.t("Предыдущее изображение"));break;case "next-page":i.click(function(){b.items.thumbs&&b.thumbnailer.next(true)});i.attr("title",b.t("Следующая страница"));break;case "prev-page":i.click(function(){b.items.thumbs&&b.thumbnailer.prev(true)});i.attr("title",b.t("Предыдущая страница"));break;case "thumbs":b.config.thumbnailer.cssID=b.config.cssID;b.config.thumbnailer.initValues=
...
i.hide();break;case "play":i.click(function(k){if(b.timer.runs){b.timer.stop();b.config.autoplay=false;i.removeClass(b.config.cssID+"-play-active");i.attr("title",b.t("Запустить автоматический показ"))}else{b.timer.start();b.config.autoplay=true;i.addClass(b.config.cssID+"-play-active");i.attr("title",b.t("Остановить автоматический показ"))}k.stopPropagation()});i.attr("title",b.t("Запустить автоматический показ"));b.config.autoplay&&i.click();break;case "albums":b.albums=new f.fn.tn3.Albums(b.data,i,b.config.cssID);i.hide();i.bind("albums_binit",
...
null||this.config.height!==null?this.resize(this.config.width,this.config.height):this.resize(this.config.initValues.width,this.config.initValues.height);if(this.items.fullscreen){this.items.fullscreen.removeClass(this.config.cssID+"-fullscreen-active");this.items.fullscreen.attr("title",this.t("Максимальный размер"))}this.config.fullOnly&&this.$c.hide();this.config.isFullScreen=false;this.trigger("fullscreen",{fullscreen:false});f(document).unbind("keyup",this.onEscape)}else{f.tn3block({message:this.$tn3,cssID:this.config.cssID});
f(window).bind("resize",f.proxy(this.onFullResize,this));this.config.fullOnly&&this.$c.show();this.config.isFullScreen=true;if(this.items.fullscreen){this.items.fullscreen.addClass(this.config.cssID+"-fullscreen-active");this.items.fullscreen.attr("title",this.t("Минимальный размер"))}this.onFullResize();e=this;this.trigger("fullscreen",{fullscreen:true})}},onEscape:function(a){a.keyCode==27&&e.fullscreen();a.keyCode==39&&e.show("next");a.keyCode==37&&e.show("prev");a.keyCode==38&&e.items.albums&&e.albums.show(0,
...
j,b):f(document).unbind("mousedown mouseup keydown keypress",b))}function b(h){var j=h.data;if(f(h.target).parents("div."+j.blockMsgClass).length>0)return true;return f(h.target).parents().children().filter("div.blockUI").length==0}var g=document.documentMode||0,i=f.browser.msie&&(f.browser.version<8&&!g||g<8),n=f.browser.msie&&/MSIE 6.0/.test(navigator.userAgent)&&!g;f.tn3block=function(h){e(h)};f.tn3unblock=function(h){a(h)};var k=undefined;f.tn3block.defaults={message:"<h1>Пожалуста, подождите...</h1>",
